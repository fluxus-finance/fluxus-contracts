...
//         .args_json(serde_json::json!({ "seed_id": token_id }))?
//         .gas(TOTAL_GAS)
//         .transact()
//         .await?;
...
//         .await?;
//     strat_creator
//         .call(&worker, token_reward2.id(), "storage_deposit")
//         .args_json(serde_json::json!({
//             "account_id": strat_creator.id()
//         }))?
//         .deposit(parse_near!("0.08 N"))
...
//     utils::register_into_contracts(&worker, exchange.as_account(), vec![farm.id()]).await?;
