...
async fn simulate_reward_distribution() -> anyhow::Result<()> {
    ...
    ///////////////////////////////////////////////////////////////////////////
        // register user
        sentry_acc.call(&worker, token_reward.id(), "storage_deposit")
        .args_json(serde_json::json!({
        ...
        // register user
        strat_creator_acc.call(&worker, token_reward.id(), "storage_deposit")
        .args_json(serde_json::json!({
    ...
    let mut fast_forward_counter: u64 = 0;
    let balance_before_sentry = i128::try_from(utils::get_balance_of(&sentry_acc,&token_reward,true,&worker, None).await?.0).unwrap();
    println!("Checking treasury balance");
    let balance_before_treasury = i128::try_from(utils::get_balance_of(treasury.as_account(),&exchange,false,&worker,Some(token_reward.id().to_string())).await?.0).unwrap();
    let balance_before_strat_creator = i128::try_from(utils::get_balance_of(&strat_creator_acc,&token_reward,true,&worker,None).await?.0).unwrap();
    ...
    let unclaimed_amount = utils::get_unclaimed_rewards(&contract,&token_id,&worker).await?;
    assert!(unclaimed_amount == 0, "ERR: Unclaimend amount should be 0 after compounding round");
    // println!("Users claimed amount: {:#?}\n", amount_claimed);
    let all_fees_amount = Percentage::from(TOTAL_PROTOCOL_FEE).apply_to(amount_claimed);
    let sentry_due_fees = i128::try_from(Percentage::from(SENTRY_FEES_PERCENT).apply_to(all_fees_amount)).unwrap();
    let strat_creator_due_fees = i128::try_from(Percentage::from(STRAT_FEES_PERCENT).apply_to(all_fees_amount)).unwrap();
    let treasury_due_fees = i128::try_from(Percentage::from(TREASURY_FEES_PERCENT).apply_to(all_fees_amount)).unwrap();  
    let balance_after_sentry = i128::try_from(utils::get_balance_of(&sentry_acc,&token_reward,true,&worker,None).await?.0).unwrap();
    let balance_after_treasury = i128::try_from(utils::get_balance_of(treasury.as_account(),&exchange,false,&worker,Some(token_reward.id().to_string())).await?.0).unwrap();
    let balance_after_strat_creator = i128::try_from(utils::get_balance_of(&strat_creator_acc,&token_reward,true,&worker,None).await?.0).unwrap();
    assert!(
    ...
    assert!(
        (balance_after_strat_creator - (strat_creator_due_fees+ balance_before_strat_creator)).abs() < 9,
